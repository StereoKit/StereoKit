...
bool window_init() {
	...
	if (!gpu_surface_create(local->ska_win, &local->skr_surface)) {
		log_fail_reason(90, log_error, "Surface creation failed");
	...
	local->surface = render_pipeline_surface_create(gpu_surface_color_format(&local->skr_surface), render_preferred_depth_fmt(), 1);
...
void window_step_begin() {
	...
	while (ska_event_poll(&evt)) {
		...
		case ska_event_window_shown: {
			...
			if (!gpu_surface_create(local->ska_win, &local->skr_surface)) break;
	...
	if (skr_surface_is_valid(&local->skr_surface)) {
		skr_vec2i_t drawable = gpu_surface_drawable_size(local->ska_win);
...
void window_step_end() {
	...
	if (acquire == skr_acquire_surface_lost) {
		...
		if (acquire == skr_acquire_needs_resize)
			skr_surface_resize(&local->skr_surface, gpu_surface_drawable_size(local->ska_win));
