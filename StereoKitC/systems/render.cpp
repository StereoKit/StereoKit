...
struct render_state_t {
	...
	array_t<bind_stack_item_t> bind_stack;
};
...
void render_check_screenshots() {
	...
	for (int32_t i = 0; i < local.screenshot_list.count; i++) {
		...
		render_bind_target_push(render_capture_surface, -1, 0);
...
void render_check_viewpoints() {
	...
	for (int32_t i = 0; i < local.viewpoint_list.count; i++) {
		...
		// Setup to render the screenshot
		render_bind_target_push(local.viewpoint_list[i].rendertarget, -1, 0);
...
void render_bind_target_push(tex_t target, int32_t layer, int32_t mip) {
	bind_stack_item_t item;
	item.surface = &target->tex;
	item.layer   = layer;
	item.mip     = mip;
	local.bind_stack.add(item);
	skg_tex_target_bind(item.surface, item.layer, item.mip);
}

///////////////////////////////////////////

void render_bind_target_pop(bool bind_top) {
	local.bind_stack.pop();
	if (bind_top) render_bind_target_bind();
}

///////////////////////////////////////////

void render_bind_target_bind() {
	if (local.bind_stack.count == 0) {
		skg_tex_target_bind(nullptr, -1, 0);
		return;
	}

	bind_stack_item_t item = local.bind_stack.last();
	skg_tex_target_bind(item.surface, item.layer, item.mip);
}
...
void render_list_draw_now(render_list_t list, tex_t to_rendertarget, matrix camera, matrix projection, rect_t viewport, render_layer_ layer_filter, render_clear_ clear) {
	render_bind_target_push(to_rendertarget, -1, 0);
