...
void hand_override_update_frame(handed_ hand) {
	hand_t *inp_hand = input_hand_ref(hand);
	inp_hand->tracked_state = button_make_state(inp_hand->tracked_state & button_state_active, override_hand[hand]);

	if (!override_hand[hand]) return;

	hand_joint_t *pose = input_hand_get_pose_buffer(hand);
	memcpy(pose, &override_joints[hand], sizeof(hand_joint_t) * 25);
	inp_hand->palm  = pose_t{ override_joints[hand][25].position, override_joints[hand][25].orientation };
	inp_hand->wrist = pose_t{ override_joints[hand][26].position, override_joints[hand][26].orientation };
