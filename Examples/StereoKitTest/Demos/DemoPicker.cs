	...
	class DemoPicker : ITest
	{
		...
		private static void ShowNodes(Model model, Material jointMaterial)
		{
			...
			for (int n = 0; n < model.Nodes.Count; n++)
			{
				...
				Mesh.Cube.Draw(jointMaterial, node.ModelTransform.Pose.ToMatrix(0.01f * scale));
				Text.Add(node.Name, node.ModelTransform * Matrix.S(scale), TextAlign.TopCenter, TextAlign.TopCenter);
			}
		}
		...
		private static void PickNodes(Model model, Material jointMaterial)
		{
			...
			if (closest != null)
			{
				Mesh.Sphere.Draw(jointMaterial, closest.ModelTransform.Pose.ToMatrix(0.025f * scale), new Color(0,1,0));
				Pose pose = closest.ModelTransform.Pose;
				UI.PushSurface(Pose.Identity);
				if (UI.Handle(closest.Name, ref pose, new Bounds(Vec3.One * 0.05f * scale)))
				{
