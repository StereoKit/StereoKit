...
class DemoMath : ITest
{
	...
	public void Step()
	{
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.S(0.4f), planeRayActive ? active:notActive);
		...
		planeMesh.Draw(Material.Default, Matrix.TRS(Vec3.Zero, Quat.LookDir(ground.normal), 0.25f), colObj);
		if (groundRay.Intersect(ground, out Vec3 groundAt))
			Mesh.Sphere.Draw(Material.Default, Matrix.TS(groundAt, 0.02f), colIntersect);
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.S(0.4f), linePlaneActive ? active : notActive);
		...
		Mesh.Sphere.Draw(Material.Default, Matrix.TS(groundLineP1, 0.01f), colTest);
		Mesh.Sphere.Draw(Material.Default, Matrix.TS(groundLineP2, 0.01f), colTest);
		bool groundLineIntersects = groundLinePlane.Intersect(groundLineP1, groundLineP2, out Vec3 groundLineAt);
		planeMesh.Draw(Material.Default, Matrix.TRS(Vec3.Zero, Quat.LookDir(groundLinePlane.normal), 0.25f), groundLineIntersects? colIntersect : colObj);
		if (groundLineIntersects)
			Mesh.Sphere.Draw(Material.Default, Matrix.TS(groundLineAt, 0.02f), colIntersect);
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.S(0.4f), sphereRayActive ? active : notActive);
		...
		if (sphereRay.Intersect(sphere, out Vec3 sphereAt))
			Mesh.Sphere.Draw(Material.Default, Matrix.TS(sphereAt, 0.02f), colIntersect);
		Mesh.Sphere.Draw(Material.Default, Matrix.TS(sphere.center, 0.25f), colObj);
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.S(0.4f), boundsRayActive ? active : notActive);
		...
		if (boundsRay.Intersect(bounds, out Vec3 boundsAt))
			Mesh.Sphere.Draw(Material.Default, Matrix.TS(boundsAt, 0.02f), colIntersect);
		Mesh.Cube.Draw(Material.Default, Matrix.TS(bounds.center, 0.25f), colObj);
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.TS(Vec3.Zero, 0.4f), boundsLineActive ? active : notActive);
		...
		Mesh.Sphere.Draw(Material.Default, Matrix.TS(boundsLineP1, 0.01f), colTest);
		Mesh.Sphere.Draw(Material.Default, Matrix.TS(boundsLineP2, 0.01f), colTest);
		Mesh.Cube  .Draw(Material.Default, Matrix.TS(boundsLine.center, 0.25f),
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.S(0.4f), meshRayActive ? active : notActive);
		suzanne  .Draw(Material.Default,  Matrix.Identity, colObj);
		...
		if (meshRay.Intersect(suzanne, out Ray meshAt)) {
			...
			Mesh.Sphere.Draw(Material.Default, Matrix.TS(meshAt.position, 0.02f), colIntersect);
		} else {
		...
		Mesh.Cube.Draw(Material.UIBox, Matrix.S(0.4f), crossActive ? active : notActive);
