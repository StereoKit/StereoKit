...
class DemoMath : IDemo
{
    ...
    {
        // Plane and Ray
        Plane ground    = new Plane (new Vec3(0,-0.25f,0), new Vec3(1,2,0));
        ...
        Vec3   boundsDir = new Vec3((float)Math.Cos(Time.Total), (float)Math.Cos(Time.Total*3)*1.5f, (float)Math.Sin(Time.Total)) * 0.35f;
        Ray    boundsRay = new Ray(bounds.center - boundsDir, boundsDir);

        Lines.Add(boundsRay.position, bounds.center, new Color32(255, 0, 0, 255), 2 * Units.mm2m);
