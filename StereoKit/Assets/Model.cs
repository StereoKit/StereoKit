...
namespace StereoKit
{
	...
	/// a Mesh instead of a Model!</summary>
	public class Model
	{
		...
		/// <returns>A Model created from the file, or null if the file failed
		/// to load!</returns>
		public static Model FromFile(string file, Shader shader = null)
		{
			IntPtr finalShader = shader == null ? IntPtr.Zero : shader._inst;
			IntPtr inst        = NativeAPI.model_create_file(file, finalShader);
		...
		/// files with references in them.</summary>
		/// <param name="filename">StereoKit still uses the filename of the 
		/// data for format discovery, but not asset Id creation. If you don't
		/// have a real filename for the data, just pass in an extension with
		/// a leading '.' character here, like ".glb".</param>
		/// <param name="data">The binary data of a model file, this is NOT 
		/// a raw array of vertex and index data!</param>
		/// <param name="shader">The shader to use for the model's materials!
		/// If null, this will automatically determine the best shader available
		/// to use.</param>
		/// <returns>A Model created from the file, or null if the file failed
		/// to load!</returns>
		...
		{
			IntPtr finalShader = shader == null ? IntPtr.Zero : shader._inst;
			IntPtr inst        = NativeAPI.model_create_mem(filename, data, data.Length, finalShader);
			return inst == IntPtr.Zero ? null : new Model(inst);
		}
