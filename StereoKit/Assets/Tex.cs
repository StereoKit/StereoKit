...
namespace StereoKit
{
    /// <summary>This is the texture asset class! This encapsulates 2D images, texture arrays,
    ...
    public class Tex
    {
        ...
        /// <summary>When sampling a texture that's stretched, or shrunk beyond its screen size, how do we
        /// handle figuring out which color to grab from the texture? Default is Linear.</summary>
        public TexSample SampleMode {
            get => NativeAPI.tex_get_sample(_inst);
            set => NativeAPI.tex_set_sample(_inst, value); }

        /// <summary>When SampleMode is set to Anisotropic, this is the number of samples the GPU takes to figure
        /// out the correct color. Default is 4, and 16 is pretty high.</summary>
        ...
        /// <summary>Sets up an empty texture container! Fill it with data using SetColors next! Creates a default unique asset Id.</summary>
        /// <param name="textureType">What type of texture is it? Just a 2D Image? A Cubemap? Should it have mip-maps?</param>
        /// <param name="textureFormat">What information is the texture composed of? 32 bit colors, 64 bit colors, etc.</param>
        ...
        /// <summary>Finds a texture that matches the given Id! Check out the DefaultIds static class for
        /// some built-in ids.</summary>
        /// <param name="id">Id of the texture asset.</param>
        /// <returns>A Tex asset with the given id, or null if none is found.</returns>
