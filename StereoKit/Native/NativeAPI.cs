...
namespace StereoKit
{
	internal static class NativeAPI
	{
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_difference (in Quaternion a, in Quaternion b);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_lookat     (in Vector3 from, in Vector3 at);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_lookat_up  (in Vector3 from, in Vector3 at, in Vector3 up);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_from_angles(float pitch_x_deg, float yaw_y_deg, float roll_z_deg);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_slerp      (in Quaternion a, in Quaternion b, float t);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_normalize  (in Quaternion a);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_inverse    (in Quaternion a);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion quat_mul        (in Quaternion a, in Quaternion b);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3    quat_mul_vec    (in Quaternion a, in Vector3 b);

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      matrix_inverse      (in Matrix4x4 a, out Matrix4x4 out_matrix);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      matrix_mul          (in Matrix4x4 a, in Matrix4x4 b, out Matrix4x4 out_matrix);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3   matrix_mul_point    (in Matrix4x4 transform, in Vector3 point);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3   matrix_mul_direction(in Matrix4x4 transform, in Vector3 direction);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Matrix4x4 matrix_trs          (in Vector3 position, in Quaternion orientation, in Vector3 scale);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      matrix_trs_out      (out Matrix4x4 out_result, in Vector3 position, in Quaternion orientation, in Vector3 scale);

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Matrix4x4 pose_matrix(in Pose pose, Vector3 scale);

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3   vec3_cross(in Vector3 a, in Vector3 b);
        
		///////////////////////////////////////////

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    plane_ray_intersect  (Plane plane, Ray ray, out Vector3 out_pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    plane_line_intersect (Plane plane, Vector3 p1, Vector3 p2, out Vector3 out_pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 plane_point_closest  (Plane plane, Vector3 pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    sphere_ray_intersect (Sphere sphere, Ray ray, out Vector3 out_pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    sphere_point_contains(Sphere sphere, Vector3 pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    bounds_ray_intersect (Bounds bounds, Ray ray, out Vector3 out_pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    bounds_point_contains(Bounds bounds, Vector3 pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool    bounds_line_contains (Bounds bounds, Vector3 linePt1, Vector3 linePt2);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 color_to_hsv(in Color color);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 color_to_lab(in Color color);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Color              sh_lookup(in SphericalHarmonics lookup, Vector3 normal);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool   mesh_ray_intersect(IntPtr mesh, Ray model_space_ray, out Vector3 out_pt);

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr mesh_gen_plane       (Vector2 dimensions, Vector3 plane_normal, Vector3 plane_top_direction, int subdivisions = 0);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr mesh_gen_cube        (Vector3 dimensions, int subdivisions = 0);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr mesh_gen_rounded_cube(Vector3 dimensions, float edge_radius, int subdivisions);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr mesh_gen_cylinder    (float diameter, float depth, Vector3 direction, int subdivisions);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr tex_gen_cubemap         (IntPtr gradient, Vector3 gradient_dir, int resolution, IntPtr lighting_info);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr tex_gen_cubemap         (IntPtr gradient, Vector3 gradient_dir, int resolution, out SphericalHarmonics lighting_info);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   material_set_vector      (IntPtr material, string name, Vector4   value);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   material_set_matrix      (IntPtr material, string name, Matrix4x4 value);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      text_add_at    (string text, in Matrix4x4 transform, int style, TextAlign position = TextAlign.Center, TextAlign align = TextAlign.Center, float off_x = 0, float off_y = 0, float off_z = 0);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      text_add_in    (string text, in Matrix4x4 transform, Vector2 size, TextFit fit, int style, TextAlign position = TextAlign.Center, TextAlign align = TextAlign.Center, float off_x = 0, float off_y = 0, float off_z = 0);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector2   text_size      (string text, int style);


		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern IntPtr solid_create          (ref Vector3 position, ref Quaternion rotation, SolidType type = SolidType.Normal);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_release         (IntPtr solid);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_add_sphere      (IntPtr solid, float diameter,               float kilograms, in Vector3 offset);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_add_box         (IntPtr solid, in Vector3 dimensions,        float kilograms, in Vector3 offset);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_add_capsule     (IntPtr solid, float diameter, float height, float kilograms, in Vector3 offset);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_move            (IntPtr solid, in Vector3 position, in Quaternion rotation);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_teleport        (IntPtr solid, in Vector3 position, in Quaternion rotation);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_set_velocity    (IntPtr solid, in Vector3 meters_per_second);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   solid_set_velocity_ang(IntPtr solid, in Vector3 radians_per_second);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Matrix4x4 model_get_transform(IntPtr model, int subset);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      model_set_transform(IntPtr model, int subset, in Matrix4x4 transform);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void      model_remove_subset(IntPtr model, int subset);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern int       model_add_subset   (IntPtr model, IntPtr mesh, IntPtr material, in Matrix4x4 transform);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   sprite_draw       (IntPtr sprite, in Matrix4x4 transform, Color32 color);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector2   sprite_get_dimensions_normalized(IntPtr sprite);

		///////////////////////////////////////////

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void line_add(Vector3 start, Vector3 end, Color32 color_start, Color32 color_end, float thickness);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Matrix4x4 render_get_cam_root  ();
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   render_set_cam_root  (Matrix4x4 cam_root);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   render_add_mesh      (IntPtr mesh, IntPtr material, in Matrix4x4 transform, Color color);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   render_add_model     (IntPtr model, in Matrix4x4 transform, Color color);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   render_screenshot    (Vector3 from_viewpt, Vector3 at, int width, int height, string file);
		//[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void render_get_device  (void **device, void **context);

		///////////////////////////////////////////

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void hierarchy_push(in Matrix4x4 transform);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 hierarchy_to_local_point    (in Vector3 world_pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 hierarchy_to_local_direction(in Vector3 world_dir);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion hierarchy_to_local_rotation (in Quaternion world_orientation);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 hierarchy_to_world_point    (in Vector3 local_pt);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector3 hierarchy_to_world_direction(in Vector3 local_dir);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Quaternion hierarchy_to_world_rotation (in Quaternion local_orientation);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void   sound_play    (IntPtr sound, Vector3 at, float volume);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void ui_layout_area (Vector3 start, Vector2 dimensions);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern Vector2 ui_area_remaining();
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void ui_reserve_box (Vector2 size);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_button_at      (string text, Vector3 window_relative_pos, Vector2 size);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_button_round_at(string text, IntPtr image_sprite, Vector3 window_relative_pos, float diameter);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_toggle_at      (string text, ref bool pressed, Vector3 window_relative_pos, Vector2 size);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_hslider_at     (string id,   ref float value, float min, float max, float step, Vector3 window_relative_pos, Vector2 size);

		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void ui_label       (string text, bool use_padding);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void ui_label_sz    (string text, Vector2 size);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void ui_image       (IntPtr sprite_image, Vector2 size);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_button      (string text);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_button_sz   (string text, Vector2 size);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_toggle_sz   (string text, ref bool pressed, Vector2 size);
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern bool ui_input       (string id,   StringBuilder buffer, int buffer_size, Vector2 size);
		...
		[DllImport(dll, CharSet = cSet, CallingConvention = call)] public static extern void ui_window_begin(string text, ref Pose pose, Vector2 size, UIWin window_type, UIMove move_type);
